    ...
    [msgs enumerateObjectsUsingBlock:^(TIMMessage * _Nonnull message, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        if (message.elemCount > 0)
        {
            ...
            if (!XOIsEmptyString(groupId)) {
                if ([elem isKindOfClass:[TIMGroupSystemElem class]]) // 群系统消息
                {
                ...
                {
                    __block TIMGroupTipsElem *tipsElem = (TIMGroupTipsElem *)elem;
                    [[XOContactManager defaultManager] getGroupInfo:groupId handler:^(TIMGroupInfo * _Nullable groupInfo) {
                        // 群资料变更
                        if (groupInfo && TIM_GROUP_TIPS_TYPE_INFO_CHANGE == tipsElem.type) {
                            if (tipsElem.groupChangeList.count > 0) {
                                ...
                                if (!XOIsEmptyString(value)) {
                                    ...
                                    // 更新通讯录中群资料
                                    [[XOContactManager defaultManager] updateGroup:groupInfo handler:^(BOOL result) {
                                        if (result) NSLog(@"更新群资料成功: %@", value);
                                        else NSLog(@"更新群资料失败: %@", value);
                                    }];
                                }
                            }
                        }
