...
- (void)layoutSubviews
{
    ...
    if ([_message isSelf]) {
        // 屏幕宽 - 10 - 头像宽
        [self.avatarImageView setOrigin:CGPointMake(self.width - 10 - self.avatarImageView.width, 13)];
        [self.messageSendStatusImageView setHidden:NO];
    }
    else {
        [self.avatarImageView setOrigin:CGPointMake(10, 13)];
...
- (void)setMessage:(TIMMessage *)message
{
    ...
    if (_message.isSelf) {
        ...
        if (profile) {
            [self.avatarImageView sd_setImageWithURL:[NSURL URLWithString:profile.faceURL] placeholderImage:[UIImage imageNamed:@"fx_default_useravatar"]];
        } else {
...
- (UIImageView *)avatarImageView
{
    if (_avatarImageView == nil) {
        ...
        [_avatarImageView setImage:[UIImage imageNamed:@"fx_default_useravatar"]];
...
- (UIImageView *)sendfailView
{
    if (_sendfailView == nil) {
        _sendfailView = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"message_sendFail"]];
...
- (void)layoutSubviews
{
    ...
    self.textLabel.font = [UIFont systemFontOfSize:14];
    self.textLabel.backgroundColor = RGBA(200, 200, 200, 1.0);
