...
@interface JTUserDefault ()
{
    NSString    *_AesSignKey;
}
@end

static JTUserDefault * __userDefault = nil;

@implementation JTUserDefault
...
+ (instancetype _Nullable )standraDefault
{
    ...
    dispatch_once(&onceToken, ^{
        __userDefault = [[JTUserDefault alloc] init];
    });
...
- (instancetype)init
{
    self = [super init];
    if (self) {
        _AesSignKey = [[JTBaseConfig defaultConfig].config.localStorageSign lowerMD5String_32];
    }
    return self;
}
