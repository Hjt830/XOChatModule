...
static JTHttpTool *__httpTool = nil;

@implementation JTHttpTool

/// 单例对象
+ (instancetype)shareTool {
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        __httpTool = [[JTHttpTool alloc] init];
    });
    return __httpTool;
}
...
- (instancetype)init
{
    ...
    if (self) {
        ...
        if (!JTIsEmptyString([self deviceVersion])) {
            [_manager.requestSerializer setValue:@"iPhone X" forHTTPHeaderField:@"_app_request_name"];
        }
        if (!JTIsEmptyString([JTKeyChainTool getToken])) {
            [_manager.requestSerializer setValue:[JTKeyChainTool getToken] forHTTPHeaderField:@"_token"];
        }
